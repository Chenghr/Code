...
bool QueensLv(ChessBoard &chess_board) {
    ...
    } while (!(nb == 0 || row_index == chess_board.board_size));
...
Evaluation ObstinateForSteps(ChessBoard &chess_board, int step_vegas) {
    ...
    if (step_vegas == 0) {
        ...
        // eva.t = eva.e / double(repeat_count);
        eva.s = global_step_count;
        ...
        if (repeat_count == 1) {
            ...
            eva.e = (eva.t - eva.s) / double(repeat_count - 1);
        }
    }
...
Evaluation RepeatOneCase(int board_size, int repeat_time, int step_vegas) {
    ...
    if (step_vegas == 0) {
        ...
        for (int repeat_index = 0; repeat_index < repeat_time; repeat_index++) {
            eva = ObstinateForSteps(chess_board, step_vegas);
            false_time = (eva.p == 1) ? false_time : false_time + 1;
            eva_sum += eva;
        }

        eva_sum /= repeat_time;

        eva_sum.e = (false_time == 0) ? 0 : eva_sum.e = eva_sum.e * repeat_time / false_time;
    }
